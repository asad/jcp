...
public class ControllerHub implements IMouseEventRelay, IChemModelRelay {
  ...
  public void setChemModel(IChemModel model) {
    ...
    public IAtom addAtomWithoutUndo(String atomType, 
		...
		IAtom newAtom;
		if (makePseudoAtom) {
		    newAtom = chemModel.getBuilder().newInstance(IPseudoAtom.class,atomType, worldCoord);
		} else {
    ...
    public IAtom addAtomWithoutUndo(String atomType, IAtom atom, IBond.Stereo stereo, boolean makePseudoAtom) {
        ...
        IBond newBond = chemModel.getBuilder().newInstance(IBond.class,atom, newAtom, CDKConstants.BONDORDER_SINGLE, stereo);
    ...
    public void setSymbol(IAtom atom, String symbol) {
		...
		if(atom instanceof IPseudoAtom){
	        IAtom newAtom = atom.getBuilder().newInstance(IAtom.class,symbol, atom.getPoint2d());
    ...
    public IRing addRing(int ringSize, Point2d worldcoord, boolean undoable) {
	    ...
	    if(undoable && getUndoRedoFactory()!=null && getUndoRedoHandler()!=null){
		    IUndoRedoable undoredo = getUndoRedoFactory().getAddAtomsAndBondsEdit(getIChemModel(), ring.getBuilder().newInstance(IAtomContainer.class,ring), null, "Ring" + " " + ringSize,this);
		    getUndoRedoHandler().postEdit(undoredo);
	    }
	    return ring;
    }
    ...
    public IRing addPhenyl(Point2d worldcoord, boolean undoable) {
        ...
        structureChanged();
	    if(undoable && getUndoRedoFactory()!=null && getUndoRedoHandler()!=null){
		    IUndoRedoable undoredo = getUndoRedoFactory().getAddAtomsAndBondsEdit(getIChemModel(), ring.getBuilder().newInstance(IAtomContainer.class,ring), null, "Benzene",this);
		    getUndoRedoHandler().postEdit(undoredo);
	    }
	    return ring;
    }
    ...
    private IRing createAttachRing(IAtomContainer sharedAtoms, int ringSize, String symbol, boolean phantom) {
	    ...
	    if(!phantom && getUndoRedoFactory()!=null && getUndoRedoHandler()!=null){
	    	IAtomContainer undoRedoContainer = newRing.getBuilder().newInstance(IAtomContainer.class,newRing);
    ...
    public void addFragment(IAtomContainer toPaste) {
	    ...
	    if(undoredofactory!=null && undoredohandler!=null){
		    IUndoRedoable undoredo = undoredofactory.getAddAtomsAndBondsEdit(getIChemModel(), toPaste.getBuilder().newInstance(IAtomContainer.class,toPaste), null, "Paste", this);
		    undoredohandler.postEdit(undoredo);
	    }
    ...
    public void mergeMolecules(Vector2d movedDistance) {
        ...
        if (movedDistance!=null && factory != null && handler != null) {
            ...
            IUndoRedoable moveundoredo = getUndoRedoFactory().getMoveAtomEdit(undoRedoContainer, movedDistance, "Move atom");
            IUndoRedoable undoredo = factory.getMergeMoleculesEdit(
                    mergedAtoms, containers, removedBondss,
                    bondsWithReplacedAtoms, movedDistance,
                    mergedPartnerAtoms, moveundoredo, "Move and merge atoms", this);
